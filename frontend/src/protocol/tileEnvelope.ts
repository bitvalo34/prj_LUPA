import { MAX_TILE_HEADER_BYTES, MAX_TILE_PAYLOAD_BYTES, type Manifest, type ParsedTile } from './types';
...
export function parseTileEnvelope(
  ...
  if (payloadBytes > Math.min(negotiatedMaxTileBytes, MAX_TILE_PAYLOAD_BYTES)) throw new Error('JPEG supera límite');

  if (header.imageId !== manifest.imageId || header.imageVersion !== manifest.imageVersion) {
    throw new Error('TILE pertenece a otra imagen');
  }
  ...
  if (header.w !== expectedWidth || header.h !== expectedHeight) throw new Error('Dimensiones TILE incoherentes');
